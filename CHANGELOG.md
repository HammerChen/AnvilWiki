...
- **老手机上修复永远到不了（缓存传播）+ 挂载失败从「空白弹窗」变「可见提示」（用户回访第五轮）**——① 真凶是 v2.14 写进 `public/_headers` 的 `/pagefind/*` 一天缓存 + 7 天 stale-while-revalidate：它把「搜索索引数据」和「UI 代码 bundle」一起缓存了，转译修复部署后老用户手机仍拿着旧的、带可选链的 pagefind-ui.js 最多 24 小时——`/pagefind/*` 改为 `max-age=0, must-revalidate`（etag 304 复验很便宜），此后任何 Pagefind 修复即时生效；② SearchButton 挂载失败不再静默：dynamic import 失败时把 `search.loadError` 文案写进弹窗（新增四语键：en/ja locale JSON + landing.ts en/zh），用户看到「搜索加载失败,请刷新页面后重试」而不是一个神秘空框。
