...
- **老手机上修复永远到不了（缓存传播）+ 挂载失败从「空白弹窗」变「可见提示」（用户回访第五轮）**——① 真凶是 v2.14 写进 `public/_headers` 的 `/pagefind/*` 一天缓存 + 7 天 stale-while-revalidate：它把「搜索索引数据」和「UI 代码 bundle」一起缓存了，转译修复部署后老用户手机仍拿着旧的、带可选链的 pagefind-ui.js 最多 24 小时——`/pagefind/*` 改为 `max-age=0, must-revalidate`；又发现 CF 区域级 Browser Cache TTL 会把浏览器侧 max-age 钳到 4 小时，故转译脚本再加**每部署内容指纹**：UI import（全部 HTML）与 entry/worker 引用（pagefind.js 内部）统一追加 `?v=<bundle 哈希>`，任何搜索修复跨部署即时生效，不再受缓存钳制；② SearchButton 挂载失败不再静默：dynamic import 失败时把 `search.loadError` 文案写进弹窗（新增四语键：en/ja locale JSON + landing.ts en/zh），用户看到「搜索加载失败,请刷新页面后重试」而不是一个神秘空框；③ 验证方式升级：route 拦截 pagefind-ui 模拟老内核故障态（兜底文案出现）+ 清浏览器缓存后真实键入（3 结果/5 精华子结果）双通过。
- **搜索弹窗「连输入框都没了」（用户回访第四轮：Pagefind 产物在老内核语法级阵亡）**——上一轮把弹窗修到能开后暴露出下一层：`pagefind-ui.js`/`pagefind.js` 等 6 个 bundle 自带可选链（42 处 `?.`），老内核（微信 X5/pre-13.4 Safari WebKit）执行即 SyntaxError → PagefindUI 永不挂载，弹窗开着但里面没有输入框。新增 `scripts/transpile-pagefind.mjs` 接入 postbuild（esbuild `target: es2018` 原地转译全部 `dist/pagefind/*.js`，import 指引/wasm/fragment 资产不动），转译后 bundle 零可选链且功能实测无损；顺带把弹窗 autofocus 收窄到精确指针设备（手机上程序化聚焦拉起软键盘，旧 iOS WebKit 会把 fixed 弹窗顶出屏外，触摸端由用户自行点输入框）；契约测试钉 postbuild 转译接线，防 Pagefind 升级/脚本误删静默复发。
- **移动端搜索「点了没反应」+ 搜到群聊精华后无法定位（用户回访第三轮，双根治）**——① 无反应三因叠加：SearchButton 的 inline 脚本含 ES2020 语法（`?.`/`??`），inline 不可转译，老内核（微信 X5/旧 WKWebView）整段 SyntaxError → 搜索全灭；landing 前批把整个 SearchButton（含 `<dialog>`）包进移动端 `hidden` 容器——现代内核 showModal 靠 top layer 逃逸 display:none 子树无恙，老内核非模态回退对话框永远 0×0 不可见——改为 field 变体触发按钮自带 `hidden sm:inline-flex`（<sm 隐藏，全宽搜索条本就替代它）、dialog 恒在渲染树；`#search-modal` 补 `position:fixed + 四向 inset + z-index`，无 showModal 的浏览器里 open 对话框也是可用覆盖层（原先按行内流落进 56px 头部行被压扁）；inline 脚本重写为 ES2018 安全并**直接绑定 `[data-open-search]`**（原是经 hidden trigger `.click()` 的双跳代理，且 inline 脚本在 DOM 解析中途执行、第二行搜索条尚不存在，绑定挂 DOMContentLoaded）；LandingLayout 代理脚本删除；非模态回退补无 `close()` 时的 `removeAttribute('open')` 关闭路径；契约测试钉 inline 脚本无 `?.`/`??` + 直接绑定，防回退。② 无法定位：Pagefind 子结果只锚到**带 id 的标题**，社群精华条目 h3 无 id → 深处命中只链到页首，千条内容无从找起——五类条目 h3 加内容哈希稳定锚点 `hi-<section>-<djb2>`（日更前置不打漂、条目文本不变则锚点不变），配 `scroll-mt` 分档让跳转落在粘性头部+chips 之下可见；顺带修 `processResult` 规范化会给带 `#` 的子结果 URL 追加尾斜杠、直接弄坏锚点跳转的隐患。
