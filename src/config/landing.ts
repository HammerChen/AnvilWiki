...
export const PROJECT_VERSION = '2.27.1';
...
const en: LandingContent = {
  ...
  announcement: {
    text: `AnvilWiki template update log (v${PROJECT_VERSION}): follow-up quality pass — UI strings are now typed end-to-end (no more blind type casts), plus a written complexity budget for future changes. Zero visible changes and zero fork migration: merge upstream and pnpm install as usual. This bar tracks template releases — details & full changelog on GitHub Releases.`,
    href: RELEASES,
...
const zh: LandingContent = {
  ...
  announcement: {
    text: `AnvilWiki 模板更新日志（v${PROJECT_VERSION}）：跟进的内功批——UI 文案全量类型化(拼错键构建期即红,不再靠盲转类型),并立下「复杂度预算」约束未来改动。零可见变化、fork 常规 merge 零迁移，照常 pnpm install 即可。本条为模板发版通告，非站点故障；详情与完整变更见 GitHub Releases。`,
    href: RELEASES,
    dismissLabel: '关闭公告',
  },
  hero: {
